...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        {
            Value singularBeta = ttData.value - (59 + 66 * (ss->ttPv && !PvNode) + 18 * (ttData.bound == BOUND_EXACT)) * depth / 63;
            ...
            if (value < singularBeta)
            {
                ...
                int doubleMargin = -2 + 204 * PvNode - 152 * !ttCapture - corrValAdj
                                 ...
                                 - 2 * (ttData.bound == BOUND_EXACT) * depth / 63;
                int tripleMargin = 70 + 279 * PvNode - 188 * !ttCapture + 81 * ss->ttPv - corrValAdj
                                 - (ss->ply > rootDepth) * 43
                                 + 10 * (ttData.bound == BOUND_EXACT) * depth / 63;
